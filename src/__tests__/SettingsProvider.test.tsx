...
function Consumer() {
  ...
  return <><Text>{settings.t('noLaunches', { lane: settings.t('new').toLowerCase() })}</Text><Text>{settings.t('quoteCheckExactIn', { slippage: '1.00' })}</Text><Text>{settings.t('tradingLocked')}</Text><Text>{settings.t('maxPositions', { count: 2 })}</Text><Text>{settings.t('copyTradeSafety')}</Text><Text>{settings.t('governanceProgress', { trades: 12, days: 3 })}</Text><Text>{settings.t('aiSafety')}</Text><Text>{settings.t('heatmapInclusion', { source: 'rpc', included: 8, excluded: 2 })}</Text><Text>{settings.t('marketIntelSafety')}</Text><Text>{settings.t('rankingEvidence', { source: 'rpc', quality: 'verified', freshness: settings.t('current') })}</Text><Text>{settings.t('walletIntelSafety')}</Text><Text>{settings.t('deviceCandidates', { count: 3 })}</Text><Text>{settings.t('researchSafety')}</Text><Text>{settings.t('operationsEvidence', { source: 'rpc', quality: 'verified', included: 9, excluded: 2 })}</Text><Text>{settings.t('operationsSafety')}</Text><Pressable accessibilityRole="button" accessibilityLabel="Vietnamese" onPress={() => settings.setLanguage('vi')}><Text>{settings.t('discover')}</Text></Pressable></>;
}
