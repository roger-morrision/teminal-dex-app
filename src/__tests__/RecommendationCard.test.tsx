...
describe('RecommendationCard', () => {
  it('opens evidence without presenting an execution action', async () => { const onOpen = jest.fn(); const screen = await render(<RecommendationCard recommendation={base} onOpen={onOpen} />); await fireEvent.press(screen.getByLabelText('Open SOL advisory evidence')); expect(onOpen).toHaveBeenCalledTimes(1); expect(screen.queryByText(/buy|execute|approve/i)).toBeNull(); });
  it('keeps expired recommendations visibly unqualified', async () => { const recommendation = { ...base, recommendationEvidence: { ...base.recommendationEvidence, status: 'expired' as const, safeForAdvisoryUse: false, expired: true } }; const screen = await render(<RecommendationCard recommendation={recommendation} onOpen={jest.fn()} />); expect(screen.getByText('Not qualified for advisory use: expired.')).toBeTruthy(); });
});
