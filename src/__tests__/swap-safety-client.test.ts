...
const policyTrace = { schemaVersion: "automation-policy-trace-v1", ownerKey: owner, intentId: "intent_123", mode: "simulation", lease: null, checks: [{ id: "instruction_integrity", passed: true, observed: hash, limit: hash }, { id: "intent_unexpired", passed: true, observed: "2026-08-23T00:00:00.000Z" }, { id: "owner_bound", passed: true, observed: owner }, { id: "simulation_succeeded", passed: true, observed: true }], blockers: [], policyHash: hash, allowed: true, executionEnabled: false };
...
describe("verified unsigned swap client", () => {
  ...
  it("requires consistent durable policy evidence for fresh v2 simulations", () => {
    const base = { schema: "swap-intent-simulation-v2", executionEnabled: false, intentId: "intent_123", replay: false, simulation: { provider: "configured-helius-rpc", slot: 7, succeeded: true, error: null, logs: [], unitsConsumed: 10, simulatedAt: 1, sigVerify: false, replaceRecentBlockhash: true, resolved: { inputMint: owner, outputMint: mint, inAmount: "123", quotedOutAmount: "456", slippageBps: 100, ownerAuthorityVerified: true, mintIdentityVerified: true, amountIdentityVerified: true, swapVariant: "route" }, policyTrace }, nextRequiredGate: "explicit_owner_confirmation" };
    expect(swapSimulationSchema.safeParse(base).success).toBe(true);
    expect(swapSimulationSchema.safeParse({ ...base, simulation: { ...base.simulation, policyTrace: undefined } }).success).toBe(false);
    expect(swapSimulationSchema.safeParse({ ...base, simulation: { ...base.simulation, policyTrace: { ...policyTrace, blockers: ["owner_bound"], allowed: true } } }).success).toBe(false);
  });
});
