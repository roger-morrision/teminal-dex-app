...
describe('TrenchCard', () => {
  it('keeps detail and quote-review intents distinct', async () => { const onDetail = jest.fn(); const onTrade = jest.fn(); const screen = await render(<TrenchCard token={token} lane="almostBonded" onDetail={onDetail} onTrade={onTrade} />); await fireEvent.press(screen.getByLabelText('Review DEX quote')); expect(onTrade).toHaveBeenCalledTimes(1); expect(onDetail).not.toHaveBeenCalled(); await fireEvent.press(screen.getByLabelText('Open DEX launch details')); expect(onDetail).toHaveBeenCalledTimes(1); });
});
