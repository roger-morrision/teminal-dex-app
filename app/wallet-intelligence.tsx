...
export default function WalletIntelligenceScreen() {
  ...
  const smart = useMemo(
    () =>
      (rankings.data?.traders ?? []).filter(
        (item) => item.badge === "Smart Money" || item.badge === "Whale",
      ),
    [rankings.data],
  );
  const smartSelected = smart.some((item) => item.address === selected)
    ? selected
    : (smart[0]?.address ?? "");
      ...
      >
        <View style={styles.header}>
          <Pressable
            ...
            accessibilityLabel={t("backWalletIntel")}
            onPress={() => router.back()}
            style={styles.back}
          >
          ...
          <View style={styles.flex}>
            <Text style={styles.eyebrow}>{t("publicAddressResearch")}</Text>
            <Text accessibilityRole="header" style={styles.title}>
              {t("walletIntelligence")}
            </Text>
          </View>
          <Text style={styles.watchOnly}>{t("watchOnly")}</Text>
        </View>
        <View accessibilityRole="summary" style={styles.notice}>
          <Ionicons name="eye" size={15} color={colors.warning} />
          <Text style={styles.noticeText}>{t("walletIntelSafety")}</Text>
        </View>
        <View accessibilityRole="tablist" style={styles.tabs}>
          <TabButton
            label={t("smartMoney")}
            accessibilityLabel={t("selectWalletIntelTab", {
              tab: t("smartMoney"),
            })}
            active={tab === "smart"}
        ...
        {tab === "smart" ? (
          ...
          <WalletTracker selected={selected} onSelect={setSelected} />
        )}
      </ScrollView>
    </SafeAreaView>
  );
  ...
  return (
    <View>
      ...
      <View style={styles.kpis}>
        <Kpi label={t("qualified")} value={String(data.length)} />
        <Kpi
          label={t("historicalPnl")}
          value={data.length ? compactUsd(total) : "—"}
        />
        <Kpi
      ...
      {!data.length ? <State text={t("noSmartWallets")} /> : null}
      {selected ? <WalletEvidence address={selected} /> : null}
    </View>
  );
}
export function SmartWalletCard({
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={t("inspectWallet", {
        badge: item.badge,
        address: item.address,
      })}
      onPress={onPress}
      style={[styles.card, styles.smartCard, active && styles.cardActive]}
    >
      <View style={styles.cardTop}>
        <View style={styles.avatar}>
          <Text style={styles.avatarText}>{item.rank}</Text>
        </View>
        <View style={styles.flex}>
          <Text style={styles.cardTitle}>
            {short(item.address)} · {item.badge}
          </Text>
          <Text style={styles.meta}>
            {t("indexedTradesReliability", {
              trades: item.trades,
              reliability:
        ...
        <View>
          <Text style={[styles.pnl, item.pnlUsd < 0 && styles.bad]}>
            {compactUsd(item.pnlUsd)}
          </Text>
          <Text style={styles.meta}>{t("historicalPnl")}</Text>
        </View>
      </View>
  ...
  return (
    <View>
      ...
      <View style={styles.form}>
        ...
        />
        <TextInput
          accessibilityLabel={t("trackedWalletLabel")}
          value={label}
...
export function TrackedWalletRow({
  item,
  active,
  onOpen,
  onRemove,
  ...
  return (
    <View style={[styles.card, active && styles.cardActive]}>
      <Pressable
        accessibilityRole="button"
        accessibilityState={{ selected: active }}
        accessibilityLabel={t("inspectTrackedWallet", { label: item.label })}
        onPress={onOpen}
        style={styles.trackedMain}
      >
        <View style={styles.avatar}>
        ...
        <View style={styles.flex}>
          ...
          <Text style={styles.signature}>{short(item.address)}</Text>
        </View>
      </Pressable>
      <Pressable
        accessibilityRole="button"
    ...
    <View style={styles.detail}>
      <View style={styles.detailTop}>
        <View style={styles.flex}>
          ...
          <Text style={styles.signature}>{short(address, 7, 6)}</Text>
        </View>
...
function Holding({
  ...
  return (
    <View style={styles.holding}>
      ...
      <View>
        <Text style={styles.value}>{compactUsd(item.valueUsd)}</Text>
        <Text style={styles.meta}>
          {item.priceUsd == null
            ? t("unpriced")
...
function Evidence({ text, warning }: { text: string; warning?: boolean }) {
  return (
    <View style={styles.evidenceBar}>
      <Ionicons
        name={warning ? "warning" : "information-circle"}
        size={14}
...
function short(value: string, lead = 5, tail = 4) {
  return value.length > lead + tail
...
const styles = StyleSheet.create({
  ...
  evidenceText: { flex: 1, color: colors.muted, fontSize: 8, lineHeight: 13 },
  kpis: {
  ...
  smartCard: { minHeight: 96, paddingBottom: 42 },
  cardActive: { borderColor: "#8c4c98", backgroundColor: "#211527" },
  cardTop: {
    flex: 1,
    flexDirection: "row",
    alignItems: "center",
  ...
  avatar: {
    ...
    justifyContent: "center",
    borderRadius: 18,
    backgroundColor: "#37213d",
  },
  avatarText: { color: "#e48cff", fontSize: 10, fontWeight: "900" },
  ...
  warning: { color: colors.muted, fontSize: 7, lineHeight: 12, marginTop: 5 },
  state: {
    minHeight: 180,
    alignItems: "center",
    justifyContent: "center",
