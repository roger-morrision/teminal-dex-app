...
const paperOperationalEvidence = { operations: { schemaVersion: 'paper-operational-health-v1', status: 'healthy', cycleStatus: 'completed', failedOrAbandoned24h: 0, openPositions: 0, qualifiedOpenPositions: 0, markCoverageApplicable: false, freshMarks: 0, freshMarkCoverage: null, leaseValid: false, reasons: [], simulationOnly: true, executionEnabled: false }, mutationHealth: { schemaVersion: 'paper-mutation-health-v2', auditedMutations: 1, qualifiedMutations: 1, excludedMutations: 0, duplicateKeys: 0, staleProcessing: 0, manualReview: 0, recoveryPolicy: 'fail_closed_no_automatic_replay', healthy: true, reasons: [], simulationOnly: true, executionEnabled: false }, jobLeaseHealth: { schemaVersion: 'paper-job-lease-health-v1', status: 'healthy_idle', observedLeases: 0, qualifiedLeases: 0, excludedLeases: 0, activeLeases: 0, expiredLeases: 0, contentionFree: true, reasonCounts: {}, simulationOnly: true, executionEnabled: false }, cycleHistoryHealth: { schemaVersion: 'paper-cycle-history-health-v2', status: 'collecting_or_invalid', observedCycles: 1, qualifiedCycles: 1, excludedCycles: 0, qualifiedTerminalCycles: 1, runningCycles: 0, contentionFree: true, fencingEvidenceRequired: true, minimumHistory: 20, historyReady: false, reasonCounts: {}, simulationOnly: true, executionEnabled: false } };
...
describe('backend client routing', () => {
  ...
  it('loads Track observations with GET only and no subscription mutation', async () => {
    jest.mocked(fetch).mockResolvedValue(jsonResponse({ notifications: [], ts: 1, error: 'notification_feed_unavailable' }, false, 503)); expect((await fetchTrackFeed()).error).toBe('notification_feed_unavailable');
    expect(fetch).toHaveBeenCalledWith('https://terminal.example/api/in-app-notifications', expect.objectContaining({ credentials: 'include' })); expect(jest.mocked(fetch).mock.calls[0]?.[1]?.method).toBeUndefined();
  });
