...
export default function WhalesScreen() {
  ...
  const evidenceCount = allEvents.length;
  ...
  return <SafeAreaView style={styles.safe} edges={["top"]}>
    <ScrollView refreshControl={mode === "alerts" ? undefined : <RefreshControl refreshing={feed.isRefetching || rankings.isRefetching} onRefresh={refresh} tintColor={colors.accent} />} contentContainerStyle={[styles.content, narrow && { paddingHorizontal: spacing.md }]}>
      {mode === "alerts" ? null : <View style={styles.searchRow}><View style={[styles.searchBox, { minHeight: 44 }]}><Ionicons name="search" size={16} color={colors.muted} /><TextInput accessibilityLabel={t("searchWhaleActivity")} placeholder={t("searchWhaleActivity")} placeholderTextColor={colors.muted} value={query} onChangeText={(value) => setQuery(value.slice(0, 80))} maxLength={80} autoCapitalize="none" autoCorrect={false} style={styles.searchInput} />{query ? <Pressable accessibilityRole="button" accessibilityLabel={t("resetWhaleFilters")} hitSlop={14} onPress={() => setQuery("")}><Ionicons name="close-circle" size={17} color={colors.muted} /></Pressable> : null}</View><View accessible accessibilityLabel={t("whaleChainScope")} style={styles.chain}><Text style={styles.chainMark}>≋</Text><Text style={styles.chainText}>SOL</Text></View></View>}
      <ScrollView horizontal showsHorizontalScrollIndicator={false} accessibilityRole="tablist" style={styles.tabRail} contentContainerStyle={[styles.tabs, { marginVertical: 0 }]}>{modes.map((item) => <Pressable key={item} accessibilityRole="tab" accessibilityState={{ selected: mode === item }} accessibilityLabel={t("selectWhaleView", { view: t(`whaleMode_${item}`) })} onPress={() => setMode(item)} style={[styles.tab, { minWidth: 88, minHeight: 44, flex: 0 }, mode === item && styles.tabActive]}><Text style={[styles.tabText, mode === item && styles.tabTextActive]}>{t(`whaleMode_${item}`)}</Text></Pressable>)}</ScrollView>
      {activityMode ? <View accessible accessibilityRole="summary" accessibilityLabel={`${evidenceCount} ${t("whaleMode_live")}. ${t("whaleEvidenceBoundary")} ${t("whaleMarketChangeBoundary")}`} style={styles.compactStatus}><View style={[styles.dot, historical && styles.stale]} /><Text numberOfLines={1} style={styles.statusText}>{evidenceCount} {t("whaleMode_live").toUpperCase()}</Text><Ionicons name="information-circle-outline" size={14} color={colors.muted} /></View> : null}
...
function Control({ label, accessibilityLabel, active, onPress }: { label: string; accessibilityLabel: string; active: boolean; onPress: () => void }) { return <Pressable accessibilityRole="radio" accessibilityLabel={accessibilityLabel} accessibilityState={{ checked: active }} hitSlop={6} onPress={onPress} style={[styles.control, { minHeight: 44 }, active && styles.controlActive]}><Text style={[styles.controlText, active && styles.controlTextActive]}>{label}</Text></Pressable>; }
  ...
  return <View>{rows.map((item) => {
    ...
    return <Pressable key={item.id} accessibilityRole="button" accessibilityLabel={t("openWhaleRelationship", { holder: whale?.label ?? t("unverifiedWhale"), holding: whale?.tokenSymbol ?? t("unknownToken"), action: t(buy ? "bought" : "sold"), amount, traded: item.tokenSymbol, dex: evidenceLabel(item.market.dex, t("unknownDex")) })} onPress={() => onOpen(item.tokenAddress)} style={[styles.card, largeText && { alignItems: "flex-start" }, { borderLeftWidth: 3, borderLeftColor: buy ? colors.positive : colors.negative }]}>
      ...
      <View style={[styles.flex, largeText && { minWidth: 0 }]}>
        <View style={styles.row}><Text numberOfLines={1} style={styles.cardTitle}>{whale?.label ?? t("unverifiedWhale")}</Text><Text style={styles.meta}>{age ? t(age.key, { count: age.count }) : t("unavailable")}</Text></View>
...
export function WalletRankings({ rows, hasEvidence, loading, refreshing, error, onRetry, onOpenAll, onOpen }: { rows: TopTrader[]; hasEvidence: boolean; loading: boolean; refreshing: boolean; error: boolean; onRetry: () => void; onOpenAll: () => void; onOpen: (address: string) => void }) {
  const { t } = useSettings();
  ...
  return <View>{rows.slice(0, 20).map((item) => {
    const holding = evidenceLabel(item.bestToken, t("unknownToken"));
    const pnlUsd = `${item.pnlUsd >= 0 ? "+" : ""}${compactUsd(item.pnlUsd)}`;
    const pnlPct = signedPercent(item.pnlPct);
    return <Pressable key={item.address} accessibilityRole="button" accessibilityLabel={`${t("inspectWallet", { badge: item.badge, address: item.address })}. ${t("bestObservedToken")}: ${holding}. ${t("totalPnl")}: ${pnlUsd} (${pnlPct}).`} onPress={() => onOpen(item.address)} style={styles.card}>
      <View style={styles.rank}><Text style={styles.rankText}>{item.rank}</Text></View>
      <View style={styles.holdingAvatar}><TokenAvatar symbol={holding} identity={`${item.address}:${holding}`} size={30} accessible={false} /></View>
      <View style={styles.flex}><Text style={styles.cardTitle}>{item.badge}</Text><Text style={styles.walletText}>{short(item.address)}</Text><Text style={styles.meta}>{item.trades} {t("observedTrades")} · {item.winRate.toFixed(1)}% {t("winRate")}</Text></View>
      <View style={styles.eventContext}><Text style={styles.holdingLabel}>{t("bestObservedToken")}</Text><Text numberOfLines={1} style={styles.holdingToken}>{holding}</Text><Text style={styles.pnlLabel}>{t("totalPnl")}</Text><Text style={[styles.amount, { color: item.pnlUsd >= 0 ? colors.positive : colors.negative }]}>{pnlUsd}</Text><Text style={[styles.pnlPercent, { color: item.pnlPct >= 0 ? colors.positive : colors.negative }]}>{pnlPct}</Text></View>
    </Pressable>;
  })}<Pressable accessibilityRole="button" accessibilityLabel={t("openWalletIntelligence")} onPress={onOpenAll} style={styles.primary}><Text style={styles.primaryText}>{t("openWalletIntelligence")}</Text></Pressable></View>;
}
