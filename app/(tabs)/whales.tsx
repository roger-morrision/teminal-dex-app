...
import { aggregateWhaleActivity, filterWhaleEvents, filterWhaleFlows, filterWhaleWalletRankings, isWhaleActivity, whaleAmountContext, type WhaleEventDirection, type WhaleEventSort, type WhaleFlow } from "@/lib/whale-activity";
...
export default function WhalesScreen() {
  ...
  return <SafeAreaView style={styles.safe} edges={["top"]}>
    <ScrollView refreshControl={<RefreshControl refreshing={feed.isRefetching || rankings.isRefetching} onRefresh={refresh} tintColor={colors.accent} />} contentContainerStyle={[styles.content, narrow && { paddingHorizontal: spacing.md }]}>
      ...
      {mode === "wallets" ? <WalletRankings rows={searchedWallets} hasEvidence={wallets.length > 0} loading={rankings.isLoading} refreshing={rankings.isFetching} error={rankings.error?.message} onRetry={() => rankings.refetch()} onOpenAll={() => router.push("/wallet-intelligence")} onOpen={(walletAddress) => router.push({ pathname: "/wallet-intelligence", params: { address: walletAddress } })} /> : mode === "alerts" ? <AlertsHandoff onOpen={() => router.push("/(tabs)/monitor")} /> : feed.isLoading ? <State text={t("loadingWhaleActivity")} /> : feed.isError ? <State error text={feed.error.message} retrying={feed.isFetching} onRetry={() => feed.refetch()} /> : !allEvents.length ? <WhaleFeedUnavailable onWallets={() => setMode("wallets")} /> : mode === "live" ? <LiveEvents rows={events} onReset={() => { setDirection("all"); setMinimumUsd(0); setSort("latest"); setQuery(""); }} onOpen={(address) => router.push({ pathname: "/token/[address]", params: { address } })} /> : <FlowList rows={searchedFlows.filter((item) => mode === "accumulating" ? item.netUsd > 0 : item.netUsd < 0)} onOpen={(address) => router.push({ pathname: "/token/[address]", params: { address } })} />}
    </ScrollView>
  </SafeAreaView>;
}
...
function WalletRankings({ rows, hasEvidence, loading, refreshing, error, onRetry, onOpenAll, onOpen }: { rows: TopTrader[]; hasEvidence: boolean; loading: boolean; refreshing: boolean; error?: string; onRetry: () => void; onOpenAll: () => void; onOpen: (address: string) => void }) {
  const { t } = useSettings();
