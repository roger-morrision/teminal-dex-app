...
export function boundedResearchNumber(value: string) { const cleaned = value.replace(/[^0-9.]/g, ''); const [whole = '', ...fractions] = cleaned.split('.'); return `${whole.slice(0, 12)}${fractions.length ? `.${fractions.join('').slice(0, 6)}` : ''}`; }
