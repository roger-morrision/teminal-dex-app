...
import { aggregateWhaleActivity, filterWhaleEvents, isWhaleActivity, whaleAmountContext, type WhaleEventDirection, type WhaleEventSort, type WhaleFlow } from "@/lib/whale-activity";
...
export default function WhalesScreen() {
  ...
  return <SafeAreaView style={styles.safe} edges={["top"]}>
    <ScrollView refreshControl={<RefreshControl refreshing={feed.isRefetching || rankings.isRefetching} onRefresh={refresh} tintColor={colors.accent} />} contentContainerStyle={[styles.content, narrow && { paddingHorizontal: spacing.md }]}>
      ...
      <View accessible accessibilityRole="summary" accessibilityLabel={`${evidenceCount} ${t(historical ? "historicalEvidence" : "whaleMode_live")}. ${t("whaleEvidenceBoundary")}`} style={styles.compactStatus}><View style={[styles.dot, historical && styles.stale]} /><Text numberOfLines={1} style={styles.statusText}>{evidenceCount} {t(historical ? "historicalEvidence" : "whaleMode_live").toUpperCase()} · {t("whaleEvidenceBoundary")}</Text><Ionicons name="information-circle-outline" size={14} color={colors.muted} /></View>
...
function MarketSnapshot({ item, compact = false }: { item: TrackNotification; compact?: boolean }) {
  ...
  return <View accessible accessibilityLabel={t("whaleMarketSnapshot", { price, marketCap, change })} style={{ flexDirection: "row", flexWrap: "wrap", alignItems: "center", gap: compact ? 4 : 6, marginTop: compact ? 5 : 7 }}><Text style={[chip, { color: colors.cyan, backgroundColor: colors.cyanDim, fontSize: 7, fontWeight: "800" }]}>{t("tokenPriceShort")} {price}</Text><Text style={[chip, { color: colors.violet, backgroundColor: colors.violetDim, fontSize: 7, fontWeight: "800" }]}>MC {marketCap}</Text><Text style={[chip, { color: positive ? colors.positive : colors.negative, backgroundColor: positive ? colors.accentDim : "#3a1820", fontSize: 7, fontWeight: "900" }]}>1h {change}</Text></View>;
}
