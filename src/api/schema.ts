...
const transactionCount = z
  .object({ buys: z.number(), sells: z.number() })
  .catch({ buys: 0, sells: 0 });
...
export const tokenDetailSchema = z
  .object({
    ...
    priceEvidence: z
      .object({
        safeForAutomation: z.boolean().optional(),
    ...
    securityEvidence: z
      .object({
        safeForAutomation: z.boolean().optional(),
        flags: z.array(z.string()).max(MAX_EVIDENCE_ROWS).optional(),
      })
      .passthrough()
      .optional(),
    riskEvidence: z
...
export const ohlcvSchema = z
  .object({
    candles: z.array(candleSchema).max(MAX_CANDLES),
    tf: z.string(),
    source: z.string(),
...
export const holdersSchema = z
  .object({
    holders: z
      ...
      .max(MAX_PAGE_ROWS),
    source: z.string().optional(),
    evidence: z.record(z.string(), evidenceMetricSchema).optional(),
  })
  .passthrough();
...
export const transactionsSchema = z
  .object({
    ...
    quality: z
      .object({
        freshness: z.string().optional(),
        completeHistory: z.boolean().optional(),
      })
      .passthrough()
      .optional(),
  })
      ...
      .array(
        z
          .object({
            ...
            boughtAt: z.number().nonnegative(),
            delaySec: z.number().nonnegative().max(300),
          })
          .passthrough(),
      )
      .max(10),
...
const providerEvidenceStatusSchema = z.enum([
  "success",
  "empty",
  "unavailable",
  "not_configured",
...
const boundedProviderStatusMapSchema = z
  .record(z.string(), providerEvidenceStatusSchema)
  .refine(
    (value) => Object.keys(value).length <= 10,
    "Too many provider statuses.",
  );
export const bubbleGraphSchema = z
  .object({
    nodes: z
      .array(
        z
    ...
    edges: z
      ...
      .max(1000),
    source: z.enum(["holders", "helius", "insightx"]),
    edgeSemantics: z.string(),
    provenance: z
      .object({
        graphSource: z.string(),
        labelSource: z.string(),
        balanceSource: z.string(),
      })
      .passthrough(),
    freshness: z
      .object({
        ...
        staleAfterMs: z.number().nonnegative(),
      })
      .passthrough(),
    completeness: z
      .object({
        transactionHistory: z.literal("partial"),
        acceptedTransfers: z.number().int().nonnegative().nullable(),
      })
      .passthrough(),
    ...
    providerEvidence: z
      .record(
        ...
        z
          .object({
            role: z.enum(["graph", "labels", "balances"]),
            status: providerEvidenceStatusSchema,
      ...
      .refine(
        (value) => Object.keys(value).length <= 10,
        "Too many provider evidence entries.",
      ),
    ts: z.number(),
  })
  .passthrough();
...
export const manipulationSchema = z
  .object({
    address: publicKeyString,
    symbol: z.string(),
    ...
    metrics: z
      .object({
        indexedSwaps: z.number().int().nonnegative(),
        indexedWallets: z.number().int().nonnegative(),
        totalIndexedVolumeUsd: z.number().nonnegative(),
        rapidRoundTripWallets: z.number().int().nonnegative(),
        roundTripWalletSharePct: z.number().min(0).max(100),
        topTraderVolumeSharePct: z.number().min(0).max(100),
        repeatedSizeVolumeSharePct: z.number().min(0).max(100),
        sampledHolders: z.number().int().nonnegative(),
        top10HolderPct: z.number().min(0).max(100),
      })
      .passthrough(),
    evidence: z
      .object({
        roundTrips: z.array(z.record(z.string(), z.unknown())).max(50),
        concentratedTraders: z
          .array(
            z
              .object({
          ...
          .max(20),
        repeatedSizes: z.array(z.record(z.string(), z.unknown())).max(20),
        holders: z
          .array(
            z
              .object({
    ...
    provenance: z
      .object({
        method: z.literal("indexed_signature_backed_heuristics"),
        observedAt: z.number(),
...
export const riskSchema = z
  .object({
    riskScore: z
      ...
      .passthrough(),
    riskEvidence: z.record(z.string(), z.unknown()).optional(),
  })
  .passthrough();
export type RiskResponse = z.infer<typeof riskSchema>;

const securitySnapshotEvidenceSchema = z
  .object({
    mintAuthority: z.string().max(100).nullable(),
    freezeAuthority: z.string().max(100).nullable(),
    isMintRenounced: z.boolean(),
    isFreezeRenounced: z.boolean(),
    holderCount: z.number().int().nonnegative().nullable(),
    buyTax: z.number().nonnegative().nullable(),
    sellTax: z.number().nonnegative().nullable(),
    isHoneypot: z.boolean().nullable(),
    isLpLocked: z.boolean().nullable(),
    devHoldingsPct: z.number().min(0).max(100).nullable(),
    topHolderPct: z.number().min(0).max(100).nullable(),
    liquidityLockPct: z.number().min(0).max(100).nullable(),
    tokenProgram: z.string().max(100).nullable().optional(),
    ...
    transferFeeBps: z
      ...
      .nullable()
      .optional(),
    permanentDelegate: z.string().max(100).nullable().optional(),
    transferHookProgramId: z.string().max(100).nullable().optional(),
    securityRiskFlags: z.array(z.string().max(100)).max(20).optional(),
  })
  .passthrough();
export const securityHistorySchema = z
  .object({
    snapshots: z
      .array(
        z
          .object({
            ...
            evidence: securitySnapshotEvidenceSchema,
          })
          .passthrough(),
      )
      .max(50),
    count: z.number().int().nonnegative().max(50),
    dataQuality: z.enum(["provider_backed", "unavailable"]),
    synthetic: z.literal(false),
  })
  .passthrough()
  .refine(
    (value) => value.count === value.snapshots.length,
    "Security snapshot count mismatch.",
  );
...
export const narrativeSchema = z
  .object({
    narrative: z
      ...
      .passthrough(),
    narrativeEvidence: z.record(z.string(), z.unknown()).optional(),
  })
  .passthrough();
export type NarrativeResponse = z.infer<typeof narrativeSchema>;

export const smartMoneySchema = z
  .object({
    signals: z.array(
      z
        .object({
          wallet: z.string(),
          action: z.enum(["accumulate", "distribute"]),
          confidence: z.number(),
          evidence: z.array(
            z
              .object({
          ...
          profitEstimate: z.number().optional(),
        })
        .passthrough(),
    ),
    walletRankingEvidence: z.record(z.string(), z.unknown()).optional(),
  })
  .passthrough();
    ...
    pairs: z.array(
      z
        .object({
          ...
          source: z.string(),
          freshness: z.string(),
          quoteIdentity: z.string(),
        })
        .passthrough(),
    ),
    dataQuality: z.string(),
    quality: z
      .object({ limitation: z.string().optional() })
      .passthrough()
      .optional(),
  })
  .passthrough();
...
const portfolioHoldingSchema = z
  .object({
    ...
    pctOfPortfolio: z.number().optional(),
    riskScore: z.number().nullable().optional(),
  })
  .passthrough();
export const portfolioAnalyticsSchema = z
  .object({
    ...
    data: z
      .object({
        address: z.string(),
        timeframe: z.string(),
        holdings: z.array(portfolioHoldingSchema).max(MAX_WALLET_HOLDINGS),
    ...
    provenance: z
      .object({
        ...
        observedAt: z.number().nullable(),
        dataQuality: z.string(),
        derived: z.array(z.string()).max(MAX_EVIDENCE_ROWS),
        unavailable: z.array(z.string()).max(MAX_EVIDENCE_ROWS),
      })
      .passthrough(),
  })
  .passthrough();
export type PortfolioAnalyticsResponse = z.infer<
  typeof portfolioAnalyticsSchema
>;

export const walletPnlSchema = z
  .object({
    pnl: z
      .object({
        ...
        tradeCount: z.number(),
        equityCurve: z
          .array(z.object({ ts: z.number(), value: z.number() }))
          .max(2000),
        provenance: z.object({
          method: z.string(),
          sources: z.array(z.string()).max(50),
          indexedSwapCount: z.number(),
        }),
        warnings: z.array(z.string()).max(MAX_EVIDENCE_ROWS),
      })
      .nullable(),
    ts: z.number().optional(),
  })
...
export const trenchesSchema = z
  .object({
    newTokens: z.array(tokenSchema).max(MAX_PAGE_ROWS),
    almostBonded: z.array(tokenSchema).max(MAX_PAGE_ROWS),
    migrated: z.array(tokenSchema).max(MAX_PAGE_ROWS),
    fetchedAt: z.number(),
    recordCount: z.number(),
    providers: z.array(z.string()).max(50),
    source: z.string(),
    dataQuality: z.string(),
    freshness: z
      .object({
  ...
  .object({
    quote: z
      .object({
        ...
        inAmountUi: z.number().positive(),
        inAmountUiExact: z.string(),
        inSymbol: z.string(),
        outAmount: z.string().regex(/^\d+$/),
        outAmountUi: z.number().positive(),
        outAmountUiExact: z.string(),
        outSymbol: z.string(),
        minOutAmount: z.string().regex(/^\d+$/),
        minOutUi: z.number().positive(),
        minOutUiExact: z.string(),
        priceImpactPct: z.number().nonnegative(),
        slippageBps: z.number().int().min(1).max(5000),
        swapUsdValue: z.number().nullable(),
        route: z.array(z.string()).min(1),
        contextSlot: z.number().int().positive(),
        real: z.literal(true),
      })
      .passthrough(),
    jupQuote: z.record(z.string(), z.unknown()),
    quotedAt: z.number(),
    ts: z.number(),
  })
  .passthrough();
...
const monitorAlertSchema = z
  ...
  .passthrough();
export const monitorAlertsSchema = z
  .object({
    alerts: z.array(monitorAlertSchema).max(MAX_PAGE_ROWS),
    ts: z.number(),
    fetchedAt: z.number(),
    source: z.string(),
    ...
    dataQuality: z.string(),
    freshness: z
      .object({ isStale: z.boolean(), staleAfterMs: z.number() })
      .passthrough(),
    error: z.string().optional(),
  })
  .passthrough();
...
export const userAlertSchema = z
  .object({
    id: z.string(),
    ...
    channels: z
      .array(z.enum(["push", "email", "inApp", "webhook", "telegram"]))
      .min(1),
    cooldownMinutes: z.number().int().nonnegative(),
    active: z.boolean(),
    lastTriggered: z.number().nullable(),
    triggerCount: z.number().int().nonnegative(),
    createdAt: z.number(),
    updatedAt: z.number(),
    persistence: z.literal("database"),
    alertEvidence: z.record(z.string(), z.unknown()).optional(),
  })
  .passthrough();
export const userAlertsSchema = z
  .object({
    success: z.literal(true),
    count: z.number(),
    data: z.array(userAlertSchema).max(MAX_PAGE_ROWS),
    persistence: z.literal("database"),
  })
  .passthrough();
export const userAlertMutationSchema = z
  .object({ success: z.literal(true), data: userAlertSchema })
  .passthrough();
export const alertDeliveriesSchema = z
  .object({
    success: z.literal(true),
    count: z.number(),
    data: z
...
export const trackFeedSchema = z
  .object({
    notifications: z
      .array(
        z
          .object({
            ...
            profitEstimateUsd: z.number().nullable().optional(),
            ...
            market: z
              .object({
                ...
                volume1h: z.number().nonnegative().nullable(),
    ...
    thresholds: z
      .object({
        surgeMinChange1h: z.number(),
        whaleMinUsd: z.number().nonnegative(),
      })
      .passthrough()
      .optional(),
    gates: z
      .object({
        surgeMinLiquidityUsd: z.number().nonnegative(),
        surgeMinVolume1hUsd: z.number().nonnegative(),
        surgeMaxDataAgeSeconds: z.number().nonnegative(),
      })
      .passthrough()
      .optional(),
    coverage: z
      .object({
        pumpfunLive: z
          .object({
            ...
            source: z.string(),
            dataQuality: z.string(),
            latestObservedAt: z.number().nullable(),
          })
          .passthrough(),
        smartMoney: z
          .object({
  ...
  .refine(
    (value) =>
      new Set(value.notifications.map((item) => item.id)).size ===
      value.notifications.length,
    "Track event IDs must be unique.",
  );
...
export const topTraderSchema = z
  .object({
    ...
    bestToken: z.string(),
    bestTokenPct: z.number(),
    badge: z.string(),
    sparkline: z.array(z.number()).max(MAX_WALLET_HOLDINGS),
  })
  .passthrough();
export const topTradersSchema = z
  .object({
    traders: z.array(topTraderSchema).max(MAX_PAGE_ROWS),
    fetchedAt: z.number(),
    recordCount: z.number(),
    requestedPeriod: z.string().optional(),
    periodApplied: z.string().optional(),
    source: z.string(),
    dataQuality: z.string(),
    provenance: z.record(z.string(), z.unknown()).optional(),
    freshness: z
      .object({
        latestSourceFetchedAt: z.number().nullable(),
        ageMs: z.number().nullable(),
        staleAfterMs: z.number(),
        isStale: z.boolean(),
      })
      .passthrough(),
    traderEvidence: z.array(z.record(z.string(), z.unknown())).optional(),
    error: z.string().optional(),
  })
  .passthrough();
...
export const copyTradeHealthSchema = z
  .object({
    service: z.literal("copytrade"),
    chain: z.literal("solana"),
    mode: z.enum(["live-awaiting-signature", "simulation", "unavailable"]),
    readiness: z.object({
      traderData: z.boolean(),
      walletMonitor: z.boolean(),
      quote: z.boolean(),
      walletSignature: z.boolean(),
      broadcast: z.boolean(),
      confirmation: z.boolean(),
      durableStorage: z.boolean(),
      automationWorker: z.boolean(),
    }),
    providers: z.object({ helius: z.boolean(), gmgn: z.boolean() }),
    recordCount: z.number(),
    checkedAt: z.number(),
  })
  .passthrough();
...
export const copyTradeConfigSchema = z
  .object({
    id: z.string(),
    userId: z.string(),
    targetWallet: publicKeyString.optional(),
    sourceWallet: publicKeyString,
    sourceWalletLabel: z.string().optional(),
    isActive: z.boolean(),
    createdAt: z.number(),
    updatedAt: z.number(),
    sizingMode: z.enum(["fixed_sol", "percentage", "proportional"]),
    fixedAmountSol: z.number().positive().optional(),
    percentage: z.number().positive().max(100).optional(),
    proportionalRatio: z.number().positive().optional(),
    maxPositionSizeSol: z.number().positive(),
    maxDailyVolumeSol: z.number().positive(),
    maxDailyLossSol: z.number().nonnegative(),
    ...
    onlyNewLaunches: z.boolean(),
    maxTokenAgeMinutes: z.number().nonnegative(),
    copySells: z.boolean(),
    copyBuys: z.boolean(),
  ...
  .passthrough();
export const copyTradeConfigsSchema = z.object({
  success: z.literal(true),
  data: z.array(copyTradeConfigSchema).max(MAX_EVIDENCE_ROWS),
});
export const copyTradeConfigMutationSchema = z.object({
  success: z.literal(true),
  data: copyTradeConfigSchema,
});
export type CopyTradeConfig = z.infer<typeof copyTradeConfigSchema>;

export const copyPositionSchema = z
  .object({
    id: z.string(),
    configId: z.string(),
    tokenAddress: publicKeyString,
    ...
    tokenName: z.string(),
    entryPrice: z.number(),
    entryAmountSol: z.number(),
    entryTokenAmount: z.number(),
    entryTxSignature: z.string(),
    entryTime: z.number(),
    sourceTxSignature: z.string(),
    executionMode: z.enum(["paper", "live"]).optional(),
    currentPrice: z.number(),
    currentValueSol: z.number(),
  ...
  .passthrough();
export const copyExecutionSchema = z
  .object({
    id: z.string(),
    userId: z.string(),
    configId: z.string(),
    ...
    requestedAmountSol: z.number().optional(),
    quotedAmountSol: z.number().optional(),
    confirmedAmountSol: z.number().optional(),
  ...
  .passthrough();
export const copyPositionsSchema = z.object({
  success: z.literal(true),
  data: z.array(copyPositionSchema).max(MAX_PAGE_ROWS),
});
export const copyExecutionsSchema = z.object({
  success: z.literal(true),
  data: z.array(copyExecutionSchema).max(MAX_WALLET_HOLDINGS),
  recordCount: z.number(),
      ...
      .array(
        z
          .object({
            ...
            createdAt: z.string().datetime(),
            recommendationEvidence: recommendationEvidenceSchema,
            outcomes: z.object({
...
export type AiRecommendation = z.infer<
  typeof aiRecommendationsSchema
>["data"]["recommendations"][number];
...
const paperPositionSchema = z
  .object({
    ...
    markStatus: z.enum(["live", "unavailable"]),
    unrealizedPnlUsd: z.number().nullable(),
    returnPct: z.number().nullable(),
  })
  .passthrough();
const closedPaperPositionSchema = z
  .object({
    id: z.string(),
    tokenAddress: publicKeyString,
    tokenSymbol: z.string(),
  ...
  .passthrough();
const paperHealthBoundarySchema = z
  .object({
...
const reasonCountsSchema = z
  ...
  .refine(
    ...
    "Too many health reason categories.",
  );
const paperOperationalHealthSchema = paperHealthBoundarySchema.extend({
  schemaVersion: z.literal("paper-operational-health-v1"),
  status: z.enum(["healthy", "degraded"]),
  cycleStatus: z.string().max(50),
  failedOrAbandoned24h: z.number().int().nonnegative(),
  openPositions: z.number().int().nonnegative(),
  qualifiedOpenPositions: z.number().int().nonnegative(),
  markCoverageApplicable: z.boolean(),
  freshMarks: z.number().int().nonnegative(),
  freshMarkCoverage: z.number().min(0).max(1).nullable(),
  leaseValid: z.boolean(),
  reasons: z.array(z.string().max(100)).max(20),
});
const paperMutationHealthSchema = paperHealthBoundarySchema.extend({
  schemaVersion: z.literal("paper-mutation-health-v2"),
  auditedMutations: z.number().int().nonnegative(),
  qualifiedMutations: z.number().int().nonnegative(),
  excludedMutations: z.number().int().nonnegative(),
  duplicateKeys: z.number().int().nonnegative(),
  staleProcessing: z.number().int().nonnegative(),
  manualReview: z.number().int().nonnegative(),
  recoveryPolicy: z.literal("fail_closed_no_automatic_replay"),
  healthy: z.boolean(),
  reasons: z.array(z.string().max(100)).max(20),
});
const paperJobLeaseHealthSchema = paperHealthBoundarySchema.extend({
  schemaVersion: z.literal("paper-job-lease-health-v1"),
  status: z.enum(["healthy_active", "healthy_idle", "degraded"]),
  observedLeases: z.number().int().nonnegative(),
  qualifiedLeases: z.number().int().nonnegative(),
  excludedLeases: z.number().int().nonnegative(),
  activeLeases: z.number().int().nonnegative(),
  expiredLeases: z.number().int().nonnegative(),
  contentionFree: z.boolean(),
  reasonCounts: reasonCountsSchema,
});
const paperCycleHistoryHealthSchema = paperHealthBoundarySchema.extend({
  schemaVersion: z.literal("paper-cycle-history-health-v2"),
  status: z.enum(["qualified_history", "collecting_or_invalid", "unavailable"]),
  observedCycles: z.number().int().nonnegative(),
  qualifiedCycles: z.number().int().nonnegative(),
  excludedCycles: z.number().int().nonnegative(),
  qualifiedTerminalCycles: z.number().int().nonnegative(),
  runningCycles: z.number().int().nonnegative(),
  contentionFree: z.boolean(),
  fencingEvidenceRequired: z.literal(true),
  minimumHistory: z.number().int().positive(),
  historyReady: z.boolean(),
  reasonCounts: reasonCountsSchema,
});
export const aiPaperReportSchema = z.object({
  success: z.literal(true),
  data: z
    .object({
      ...
      summary: z
        .object({
          ...
          markCoverage: z.number().min(0).max(1),
          unavailableMarks: z.number().int().nonnegative(),
        })
        .passthrough(),
      analytics: z
        .object({
          ...
          totalSlippageCostUsd: z.number(),
        })
        .passthrough(),
      risk: z
        .object({
          entriesAllowed: z.boolean(),
          dailyLossLimitHit: z.boolean(),
      ...
      readiness: z
        .object({
          status: z.string(),
          executionEnabled: z.literal(false),
          killSwitch: z.literal(true),
          note: z.string(),
          checks: z.record(z.string(), z.boolean()),
        })
        .passthrough(),
      operations: paperOperationalHealthSchema,
      mutationHealth: paperMutationHealthSchema,
      jobLeaseHealth: paperJobLeaseHealthSchema,
      cycleHistoryHealth: paperCycleHistoryHealthSchema,
      positions: z.array(paperPositionSchema).max(MAX_PAGE_ROWS),
      closedTrades: z.array(closedPaperPositionSchema).max(MAX_PAGE_ROWS),
      dailyPerformance: z
        .array(
          z
            .object({
        ...
        .array(
          z
            .object({
              ...
              monitoredMinutes: z.number(),
              riskScore: z.number().nullable(),
              socialScore: z.number().nullable(),
              lifecycle: z.string(),
              status: z.string(),
              requiredMonitoringMinutes: z.number(),
            })
            .passthrough(),
        )
        .max(MAX_PAGE_ROWS),
...
export const aiPlatformSchema = z.object({
  success: z.literal(true),
  data: z
    .object({
      schema: z.literal("ai-platform-readiness-v1"),
      phases: z.array(
        z.object({
          phase: z.number().int(),
          title: z.string(),
...
const freshnessSchema = z
  .object({
    ...
    staleAfterMs: z.number().nonnegative(),
    ageMs: z.number().nullable().optional(),
    latestSourceFetchedAt: z.number().nullable().optional(),
...
export const marketSignalSchema = z
  .object({
    ...
    token: z.string(),
    tokenAddress: publicKeyString.optional(),
    description: z.string(),
    time: z.string(),
    ts: z.number().optional(),
    wallet: publicKeyString.optional(),
...
export const signalsSchema = z
  .object({
    signals: z.array(marketSignalSchema).max(MAX_MARKET_ROWS),
    signalEvidence: z
      .array(z.record(z.string(), z.unknown()))
      .max(MAX_EVIDENCE_ROWS)
      .optional(),
    fetchedAt: z.number(),
    recordCount: z.number().int().nonnegative(),
...
export const heatmapSchema = z
  .object({
      ...
      .array(
        z
          .object({
            ...
            trustFlags: z.array(z.string()).max(50),
            source: z.string().optional(),
          })
          .passthrough(),
      )
    ...
    trustSummary: z
      .object({
        warningRecordCount: z.number().int().nonnegative(),
        lowLiquidityCount: z.number().int().nonnegative(),
        noPriceCount: z.number().int().nonnegative(),
        transactionCountUnavailable: z.number().int().nonnegative(),
        suspiciousMetadataCount: z.number().int().nonnegative(),
        nonCanonicalMintCount: z.number().int().nonnegative(),
        incompleteMetricCount: z.number().int().nonnegative(),
...
export const claimMonitorSchema = z
  .object({
    generatedAt: z.number(),
    health: z.enum(["healthy", "degraded", "unhealthy"]),
    source: z.literal("solana-rpc"),
    mode: z.literal("rpc-polling"),
    programId: publicKeyString,
    programIds: z.array(publicKeyString).max(20),
    rpcEndpoint: z.string(),
    signaturesScanned: z.number().int().nonnegative(),
    claimsDetected: z.number().int().nonnegative(),
    firstClaims: z.number().int().nonnegative(),
    fakeClaims: z.number().int().nonnegative(),
    events: z
      .array(
        z
          .object({
            ...
            platform: z.enum(["github", "unknown"]),
            amountLamports: z.number().nonnegative().nullable(),
            amountSol: z.number().nonnegative().nullable(),
            feePayer: publicKeyString.nullable(),
            status: z.enum([
              "confirmed",
              "failed",
              "fake_or_unpaid",
            ...
            isFakeClaim: z.boolean(),
            logs: z.array(z.string()).max(MAX_EVIDENCE_ROWS),
...
export const walletHoldingsSchema = z
  .object({
    wallet: z
      .object({
        address: publicKeyString,
          ...
          .array(
            z
              .object({
                ...
                pctOfPortfolio: z.number().nonnegative().max(100),
              })
              .passthrough(),
          )
          .max(MAX_WALLET_HOLDINGS),
        ...
        solValueUsd: z.number().nonnegative(),
      })
      .passthrough(),
    ts: z.number(),
  })
...
const feedRecordsSchema = z
  .object({
    pairs: z.number().int().nonnegative(),
    transactions: z.number().int().nonnegative(),
    candles: z.number().int().nonnegative(),
    total: z.number().int().nonnegative(),
    lastPersistedAt: z.number().nullable().optional(),
    persistenceAgeMs: z.number().nonnegative().nullable().optional(),
    freshness: z.enum(["fresh", "stale", "unavailable"]).optional(),
  })
  .passthrough();
const feedRateLimitSchema = z
  .object({
    ...
    lastRateLimitedAt: z.number().nullable(),
    lastRetryAfterMs: z.number().nonnegative().nullable(),
  })
  .passthrough();
const feedConnectionRuntimeSchema = z
  .object({
    configured: z.boolean().optional(),
    connected: z.boolean(),
    subscriptionCount: z.number().int().nonnegative().optional(),
    subscriptions: z.array(z.string().max(100)).max(100).optional(),
    recordCount: z.number().int().nonnegative(),
    lastSuccessAt: z.string().datetime().nullable(),
    lastError: z.string().max(500).nullable(),
    reconnectAttempts: z.number().int().nonnegative().optional(),
    subscriptionErrors: z.number().int().nonnegative().optional(),
    subscriptionHealthy: z.boolean().optional(),
    trafficHealthy: z.boolean().optional(),
  })
  .passthrough();
const feedCounterSchema = z
  .object({
    ...
    quality: z.enum(["healthy", "degraded"]),
    cooldownRemainingMs: z.number().nonnegative(),
  })
  .passthrough();
const feedEventBusSchema = z
  .object({
    published: z.number().int().nonnegative(),
    persisted: z.number().int().nonnegative(),
    droppedDuplicates: z.number().int().nonnegative(),
    droppedInvalidTimestamps: z.number().int().nonnegative(),
    persistFailures: z.number().int().nonnegative(),
    persistenceDrops: z.number().int().nonnegative(),
    pendingPersistence: z.number().int().nonnegative(),
    lastEventAt: z.string().datetime().nullable(),
  })
  .passthrough();
export const feedConnectionsSchema = z
  .object({
    success: z.literal(true),
    chain: z.literal("solana"),
    generatedAt: z.number(),
    source: z.literal("runtime_provider_inventory"),
    runtimeScope: z.enum(["durable-indexer-heartbeat", "local-process"]),
      ...
      .array(
        z
          .object({
            id: z.string(),
            label: z.string(),
            method: z.enum(["api", "websocket", "rpc", "storage"]),
            status: z.string(),
            health: z.string(),
            receiving: z.boolean(),
            deliveryStatus: z.string(),
            configured: z.boolean(),
            records: feedRecordsSchema,
            rateLimit: feedRateLimitSchema.nullable(),
            subscription: z
              .object({
                connected: z.boolean(),
              ...
              .nullable(),
            runtime: feedConnectionRuntimeSchema.optional(),
          })
          .passthrough(),
      )
      .max(50),
    runtime: z
      .object({ eventBus: feedEventBusSchema, onchainTicks: feedCounterSchema })
      .passthrough(),
    ingestionJobs: z
      .array(
        z
          .object({
            ...
            tokensFailed: z.number().int().nonnegative(),
...
export const feedDiagnosticsSchema = z
  .object({
    generatedAt: z.number(),
    runtimeScope: z.enum(["durable-indexer-heartbeat", "local-process"]),
    realtimeEvidence: z
      .object({ timestamp: z.string().nullable(), timestampValid: z.boolean() })
      .passthrough(),
    quality: z
      .object({
        rpc: z.string(),
        websocket: z.string(),
        eventPersistence: z.string(),
        decoder: z.string(),
      })
      .passthrough(),
