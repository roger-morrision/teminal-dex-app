...
export default function WhalesScreen() {
  ...
  return <SafeAreaView style={styles.safe} edges={["top"]}>
    <ScrollView refreshControl={<RefreshControl refreshing={feed.isRefetching || rankings.isRefetching} onRefresh={refresh} tintColor={colors.accent} />} contentContainerStyle={[styles.content, narrow && { paddingHorizontal: spacing.md }]}>
      ...
      {mode === "wallets" ? <WalletRankings rows={wallets} loading={rankings.isLoading} refreshing={rankings.isFetching} error={rankings.error?.message} onRetry={() => rankings.refetch()} onOpen={(walletAddress) => router.push({ pathname: "/wallet-intelligence", params: { address: walletAddress } })} /> : mode === "alerts" ? <AlertsHandoff onOpen={() => router.push("/(tabs)/monitor")} /> : feed.isLoading ? <State text={t("loadingWhaleActivity")} /> : feed.isError ? <State error text={feed.error.message} retrying={feed.isFetching} onRetry={() => feed.refetch()} /> : !allEvents.length ? <WhaleFeedUnavailable onWallets={() => setMode("wallets")} /> : mode === "live" ? <LiveEvents rows={events} onReset={() => { setDirection("all"); setMinimumUsd(0); setSort("latest"); setQuery(""); }} onOpen={(address) => router.push({ pathname: "/token/[address]", params: { address } })} /> : <FlowList rows={flows.filter((item) => mode === "accumulating" ? item.netUsd > 0 : item.netUsd < 0)} onOpen={(address) => router.push({ pathname: "/token/[address]", params: { address } })} />}
    </ScrollView>
  </SafeAreaView>;
}
...
function WalletRankings({ rows, loading, refreshing, error, onRetry, onOpen }: { rows: TopTrader[]; loading: boolean; refreshing: boolean; error?: string; onRetry: () => void; onOpen: (address: string) => void }) {
  const { t } = useSettings();
